...
(*Renvoie une solution al�atoire*)
...
(*ajoute le changement li� a la solution suivante au tabu et enleve des elements a la liste si sa taille maximale est atteinte*)
...
let main_tabu = fun t_size p_tab g_tab max_bad_iter max_iter fact_delta fact_conflit->   (* argument = taille de la liste tabu*)
  ...
  let iter=ref 0 in
  while !nbr_bad_iter <= max_bad_iter && !iter < max_iter do     (* nbr_bad_iter = nbr d'it�ratins qui n'augmentent pas la fonction objectif *)
    iter:=!iter+1;
    let scourant_voisinage = ref (Neighbours.voisinage  !s_courant p_tab !iter) in (*calcul du voisinage*)

    (* voisinage = liste de (solution, changement) o� changement  = array d'array de int, soit un switch soit un d�placement*)
